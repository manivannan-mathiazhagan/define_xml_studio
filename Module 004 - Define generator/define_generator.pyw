...
# -*- coding: utf-8 -*-
"""
Clinical Metadata Studio - Module 2 / Define Studio
Author: Manivannan Mathialagan

Purpose
-------
Python-native metadata editor, CT/Formats generator, VLM generator and Define-XML
publisher for SDTM/ADaM. This script is designed as a SAS-free replacement for
the define generation part while preserving the rules used in the current SAS
macros. Specification input is SharePoint-only using get_spreadsheet.py. Dataset input is XPT-only.

Major rules implemented
-----------------------
1. Only KEEP=1 metadata rows are used for define.xml, formats and VLM.
2. Length is taken from dataset metadata whenever the dataset variable exists.
3. Editable metadata fields are only Type, Format, Origin, Comments.
4. Non-editable metadata columns are frozen/colored differently in the GUI.
5. ID Var controls KeySequence in define.xml.
6. Define versions 2.0 and 2.1 are selectable.
7. SDTM VLM is generated automatically only for --ORRES and SUPP-- QVAL.
8. ADaM VLM is generated by PARAMCD/PARAM using one result variable per where-clause:
   AVAL when values are numeric; otherwise AVALC when character/non-numeric values are present.
9. Formats are generated only for KEEP=1 rows where the GUI Format field is non-missing.
   The Format field is initialized from the spec Control or Format column only; users may
   add/edit it in the GUI before refreshing Formats. Code/decode pair rules are then used
   only for those selected format variables.
10. Origin='Derived' requires Comments and writes MethodDef.
11. Origin='Assigned' requires Comments and writes CommentDef.
12. Origin containing CRF page references creates aCRF PDF page links.
13. SDTM includes acrf.pdf and csdrg.pdf links. ADaM includes adrg.pdf link.

Run
---
python define_studio_module2.pyw

Dependencies
------------
pandas, openpyxl, PyQt5, pyreadstat
Optional: lxml is not required; standard xml.etree is used.
"""
...
def dataset_class_sort_value(ds, domain_lookup):
    ds = safe_upper(ds)
...
def sas_best_text(value):
    """Display numeric values like SAS BEST.: 1.0 -> 1, 8.1 -> 8.1.

    Character values are preserved exactly after strip, so values such as 01 are not changed.
    """
    if value is None:
        return ""
    try:
...
def infer_codelist_datatype_from_codes(code_values, fallback="text"):
    """Infer Define-XML CodeList DataType from the actual Code values.

    Rules used for P21 OD0080 consistency:
    - If any nonblank code is not numeric, return text.
    - If all nonblank codes are numeric whole numbers, return integer.
    - If all nonblank codes are numeric and at least one has decimals, return float.
    - If no usable codes exist, return fallback.

    This is intentionally codelist-level logic. The variable ItemDef datatype is
    inferred separately from XPT metadata/values; the CodeList datatype must be
    based on the codelist coded values that will actually be written.
    """
    vals = []
    has_decimal = False
...
def apply_cdisc_synonyms_from_ct_table(formats_df, ct_df):
    ...
    for _, r in ct.iterrows():
        ...
        # Also add acronym-like final token from codelist name if possible.
...
class DefineXmlWriter:
    def __init__(self, standard, define_version, ig_version, ct_version, study_oid, study_name,
                 ...
                 csdrg_file="csdrg.pdf", adrg_file="adrg.pdf", domains_df=None):
    ...
    def build(self):
        ...
        if self.include_acrf and self.standard == "SDTM":
            acrf = ET.SubElement(mdv, self.dq("AnnotatedCRF"))
            ET.SubElement(acrf, self.dq("DocumentRef"), {"leafID": "LF.blankcrf"})
        if self.include_rg:
            suppdoc = ET.SubElement(mdv, self.dq("SupplementalDoc"))
            ET.SubElement(suppdoc, self.dq("DocumentRef"), {"leafID": "LF.ReviewersGuide"})
    ...
    def add_itemgroups(self, mdv):
        ...
        dataset_order = sorted(
            ...
            key=lambda x: (dataset_class_sort_value(x, self.domain_lookup), x)
        )
        for ds in dataset_order:
            ...
            if documentation and not documentation.lower().endswith((".pdf", ".html", ".htm", ".docx", ".rtf", ".txt")):
    ...
    def add_itemdefs(self, mdv):
        for _, row in self.metadata_df.iterrows():
            ...
            # Define-XML XSL displays variable comments from ItemDef/@def:CommentOID,
            # not from ItemRef.  Add this for Assigned/eDT/Predecessor/etc. comments.
            # Derived comments continue to become MethodDef via ItemRef/@MethodOID.
            if comments and origin != "DERIVED":
    ...
    def add_origin(self, item, row):
        ...
        if upper in known:
            ET.SubElement(item, self.dq("Origin"), {"Type": known[upper]})
            return
    ...
    def add_methods_and_comments(self, mdv):
        ...
        # Variable-level methods/comments
        for _, row in self.metadata_df.iterrows():
    ...
    def add_leaves(self, mdv):
        if self.standard == "SDTM" and self.include_acrf:
            leaf = ET.SubElement(mdv, self.dq("leaf"), {"ID": "LF.blankcrf", self.xq("href"): "acrf.pdf"})
            ET.SubElement(leaf, self.dq("title")).text = "Annotated Case Report Form"
        if self.include_rg:
            href = "csdrg.pdf" if self.standard == "SDTM" else "adrg.pdf"
            title = "SDTM Reviewer's Guide" if self.standard == "SDTM" else "ADaM Reviewer's Guide"
            leaf = ET.SubElement(mdv, self.dq("leaf"), {"ID": "LF.ReviewersGuide", self.xq("href"): href})
...
class DefineStudio(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Clinical Metadata Studio - Module 2 / Define Studio")
    ...
    def build_ui(self):
        ...
        title = QtWidgets.QLabel("Clinical Metadata Studio  •  SharePoint ValueMetadata")
        title.setObjectName("Title")
        subtitle = QtWidgets.QLabel("Module 2 • Python Define Studio • Metadata Editor • CT/Formats • VLM • Define-XML 2.0/2.1")
        ...
        self.set_status("Ready - SharePoint ValueMetadata mode loaded", state="info")
    ...
    def generate_formats(self):
        ...
        self.ct_alias_map = fetch_cdisc_library_ct_map(
            self.cdisc_api_key,
        ...
        for _, m in meta.iterrows():
            ...
            if possible_code and possible_code in cols_upper:
                code_var = spec_var
                decode_var = ""
